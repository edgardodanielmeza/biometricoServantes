...
from ..reports.pdf_generator import generate_incidents_report, generate_incidents_excel_report
from .pdf_viewer import PDFViewer
from .print_utils import imprimir_pdf

class IncidentsReportWindow(tk.Toplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("Reporte de Incidencias")
    ...
    def generate_report(self):
        ...
        datos = self.controlador.obtener_incidencias_por_dia(fecha)
        if not datos:
            messagebox.showinfo("Información", "No se encontraron incidencias para la fecha seleccionada.")
            return

        self.show_preview(fecha, datos)

    def show_preview(self, date, incidents_data):
        with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as temp_file:
            temp_path = temp_file.name

        # This will be the new function in pdf_generator
        if not generate_incidents_report(temp_path, date, incidents_data):
            os.unlink(temp_path)
            messagebox.showerror("Error", "No se pudo generar el reporte en PDF.")
            return

        preview_window = tk.Toplevel(self)
        preview_window.title(f"Previsualización - Reporte de Incidencias")
        try:
        ...
        def save_pdf():
            filename = filedialog.asksaveasfilename(
                ...
                initialfile=f"Reporte_Incidencias_{date}.pdf"
            )
            if filename:
        ...
        def export_excel():
            filename = filedialog.asksaveasfilename(
                ...
                initialfile=f"Reporte_Incidencias_{date}.xlsx"
            )
            if filename:
                if generate_incidents_excel_report(filename, date, incidents_data):
